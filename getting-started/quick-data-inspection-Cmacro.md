...
Even though this macro uses only ROOT classes directly, it should be executed with `restRoot`, because the input files contain REST objects and dictionaries that must be loaded before the file is opened. For a more interactive first look at the same kind of file, see the [`restRoot` inspection page](quick-data-inspection-restRoot.md).

You can launch the macro by typing `.L 1_macro_histograms_example.C` and then call the function with `run_duration()`.
